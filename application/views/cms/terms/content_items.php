    ...
    <div class="row justify-content-between align-items-center">
        <div class="col-md-6 col-6 d-flex align-items-center justify-content-between justify-content-md-start">
            <div class="d-inline-block">
                <h5 class="h4 d-inline-block font-weight-400 mb-0 text-white">Term & Condition - <?=$core['subject']?></h5>
            </div>
        </div>
        <div class="col-md-6 col-6 d-flex align-items-center justify-content-end justify-content-md-end">
            <div class="actions actions-dark d-inline-block">
                ...
                <a href="<?=base_url('cms/terms')?>" class="text-white ml-md-4  ">
...
<div class="modal fade" id="modalUpdateStatus" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <form class="modal-content" action="<?php echo base_url('cms/terms/save_content/'.$core['id']); ?>" method="post" enctype="multipart/form-data">
...
<div class="modal fade" id="modalUpdateData" tabindex="-1" role="dialog" data-backdrop="static" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <form class="modal-content" action="<?php echo base_url('cms/terms/update_content'); ?>" id="updateb" method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" id="idb">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel1">Edit Term & Condition</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
    ...
    $(function () {
        $('.publishb').click(function(e){
            ...
            // console.log(status) 
            $.ajax({
                url: "<?php echo base_url('cms/terms/publish_content'); ?>",
                type: "post",
                data: {
                    id: id,
                    status: !status?1:0
            ...
            MessageBox.confirm({
                ...
                callback: function(btn) {
                    if(btn === "Y") {
                        ...
                        $.ajax({
                            url: "<?php echo base_url('cms/terms/delete_content'); ?>",
                            type: "post",
                            data: {id: id},
                            dataType: "json",
                            success: function (resp) {
        ...
        $('#updateb').submit(function(e){
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url('cms/terms/update_content'); ?>",
                cache: false,
                contentType: false,
                processData: false,
                data: formData,
                success: function(data) {
                    swal.fire({
                        title: "Success!",
                        text: "Success to save update terms!",
